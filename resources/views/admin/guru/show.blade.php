...
@section('heading')
  List of teachers {{ $mapel->nama_mapel }}
@endsection
@section('page')
                ...
                <tr>
                    ...
                    <td>
                        <form action="{{ route('guru.destroy', $data->id) }}" method="post">
                            @csrf
                            @method('delete')
                            <a href="{{ route('guru.show', Crypt::encrypt($data->id)) }}" class="btn btn-info btn-sm mt-2"><i class="nav-icon fas fa-id-card"></i> &nbsp; Details</a>
